...
__global__ void sha256_cuda(uint8_t *prefix, size_t prefixlen, uint8_t *minvalue, uint8_t *results) {
	...
	sha256_init(&ctx);
	sha256_update(&ctx, prefix, prefixlen);
	...
	results[nonce] = 0;
	for (int i = 0; i < 32; i++) {
		if (digest[i] > minvalue[i]) {
			break;
		} else if (digest[i] < minvalue[i]) {
			results[nonce] = 1;
			break;
		}
	}
